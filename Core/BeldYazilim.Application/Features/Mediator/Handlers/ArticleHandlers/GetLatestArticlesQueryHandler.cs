...
{
    public class GetLatestArticlesQueryHandler : IRequestHandler<GetLatestArticlesQuery, List<GetLatestArticlesQueryResult>>
    {
        private readonly IArticleRepository _repository;

        public GetLatestArticlesQueryHandler(IArticleRepository repository)
        {
            _repository = repository;
        }
        public Task<List<GetLatestArticlesQueryResult>> Handle(GetLatestArticlesQuery request, CancellationToken cancellationToken)
        {
            var values = _repository.GetLatestArticles(4);
